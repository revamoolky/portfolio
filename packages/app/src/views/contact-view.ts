...
import { Msg, ContactInput } from "../messages";
...
export class ContactViewElement extends View<Model, Msg> {
  ...
  get status() {
    return this.model.contactSubmissionStatus;
  }

  async handleContactSubmit(e: Event) {
    e.preventDefault();
    this.isSubmitting = true;
    this.dispatchMessage(["contact/status", { status: "" }]);
    ...
    this.dispatchMessage(["contact/submit", { contact: formData }]);
    setTimeout(() => {
      this.isSubmitting = false;
    }, 1000); // optional UI delay
    form.reset();
  }
  ...
  static styles = css`/* (use your existing styles here) */`;
}
